...
验证方式 & 组件级自检 21 项 + 前端渲染自检 5 项 + 三组 API 真实执行实验 \\
...
\begin{center}
\kpi{96.7\%}{技能检索召回\\（基线 77.5\%）}
\hspace{1.2em}
\kpi{+15pp}{领域要点覆盖率\\（82.1\% → 97.4\%）}
\hspace{1.2em}
...
无技能基线的盲评均分已达 8.32/10；在「老虎机选择 vs 随机选择」的对照中，两者的实测奖励差距只有 0.011。
任务本身是「给出研究执行方案」，强模型不做任何准备也能拿到高分，
技能补的特定工具用法与陷阱被压缩了。

\textbf{发现三：技能采纳率是一个被普遍忽视的独立指标。}
即使把技能全文摆在 Agent 面前，也只有 \textbf{67\%–84\%} 的技能被真正写进方案。
...
\item \textbf{按「检索层 → 进化闭环 → 调度强化学习」分三阶段落地}，每阶段独立可验证、可回退。
   检索层的收益在本报告中已被直接证实（召回 77.5\% → 96.7\%），风险最低，适合先做。
...
\textbf{Fabric 完整方案}（＋关系扩展＋LLM 重排） & \textbf{96.7\%} & \textbf{90.0\%} & \textbf{95.0\%} \\
\bottomrule
\end{tabular}
...
\addplot[fill=inkjade, draw=inkjade] coordinates {(BM25,77.5) (混合检索,78.3) (Fabric,96.7)};
\addplot[fill=inkgold, draw=inkgold] coordinates {(BM25,50.0) (混合检索,55.0) (Fabric,90.0)};
\addplot[fill=inkaccent, draw=inkaccent] coordinates {(BM25,60.0) (混合检索,62.5) (Fabric,95.0)};
...
漏掉的那个技能往往正是让流程跑通的关键。加入关系图扩展后，完全覆盖率从 55\% 跃升到 90\%，
这说明：在大技能池里，\textbf{仅靠「和查询像不像」是不够的，还要沿技能间的依赖与组合关系把工作流上下游补齐}。
编排完整度从 60\% 到 95\% 则说明，技能集选对之后，执行顺序基本可以自动推导出来。
...
混合检索 ＋ 技能全文 & 8.77 & \textbf{98.3\%} & 66.7\% & +5.4\% & ¥0.078 \\
\textbf{完整方案}（扩展＋重排＋全文） & \textbf{8.81} & 97.4\% & 77.2\% & \textbf{+5.8\%} & ¥0.084 \\
\bottomrule
\end{tabular}
...
完整方案 12 个任务中 \textbf{7 胜 1 平}，要点覆盖率 7 胜。
...
\addplot[fill=inkjade, draw=inkjade] coordinates {
  (82.1,无技能) (93.1,仅技能卡) (98.3,混合检索) (97.4,完整方案)};
\end{axis}
\end{tikzpicture}
\end{center}

\textbf{结论。} 技能带来的最大改变不是「方案更好看」，而是「专业操作要点被覆盖」。
覆盖率提升约 15 个百分点，而盲评均分只提升 5.8\%。
另外，即使把技能全文交给 Agent，也只有 67\%–84\% 的技能被真正用上。
...
\midrule
\textbf{LinUCB 老虎机} & \textbf{0.831} & literature-review & 51 → 54 & 3 & ¥0.138 \\
随机选择（消融） & 0.820 & statistical-testing & 51 → 54 & 3 & ¥0.145 \\
\bottomrule
\end{tabular}
...
3 & 文献 & \texttt{literature-review} & \texttt{citation-verify} \\
4 & 临床 & \texttt{clinical-feature-engineering} & \texttt{statistical-testing} \\
5 & 因果推断 & \texttt{econometric-panel} & \texttt{causal-inference} \\
6 & 分子对接 & \texttt{molecular-docking} & \texttt{md-simulation} \\
\bottomrule
\end{tabular}
...
随机选择则经常落到通用或邻域技能上。但\textbf{实测奖励的差距只有 0.011}，
在 6 轮样本量下不构成显著结论。原因与实验二同源：任务本身存在天花板。

进化侧则是确定生效的：两个对照组各蒸馏出 3 个通过准入过滤的新技能
（如 \texttt{composition-only-formation-energy-extrapolation}、
\texttt{leakage-safe-survival-feature-pipeline}），技能库从 51 增长到 54，
...
\midrule
领域要点覆盖率 & 82.1\% & 97.4\% \quad（\textbf{+15.3 个百分点}） \\
LLM 盲评均分 & 8.32 & 8.81 \quad（\textbf{+5.8\%}） \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
\item 实验二：无技能基线已拿到 8.32/10，四档之间的最大差距只有 0.49 分。
\item 实验三：LinUCB 与随机选择的平均实测奖励差距只有 \textbf{0.011}，
...
\textbf{本报告实验一的数字（召回 77.5\% → 96.7\%）就是这条路径的直接依据。}
...
\item \textbf{组件自检}：\texttt{python verify.py}（21 项，不需要密钥）；
